...
export default function MobileOptimizedBooking() {
    ...
    useEffect(() => {
        const init = async () => {
            await Promise.all([fetchStylists(), fetchServices()]);
            generateWeekDates();
    ...
    // Handle initial service selection from URL
    useEffect(() => {
        if (initialLoaded && preselectedServiceId && services.length > 0) {
    ...
    }, [initialLoaded, preselectedServiceId, services]);
