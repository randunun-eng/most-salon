...
export async function runBookingStateMachine(
    ...
    if (step === 'ask_time') {
        const wantAnyStylest = /\b(any|another|who else|anyone else|other stylist|another stylist|someone else|whoever|different stylist|any stylist|anyone|other)\b/i.test(userMessage);
