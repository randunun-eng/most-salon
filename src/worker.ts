...
export default {
    async fetch(request: Request, env: Env): Promise<Response> {
        ...
        try {
            ...
            if (path === '/api/stylists/calendar' && method === 'GET') {
                const stylistId = url.searchParams.get('id');
                if (!stylistId) return json({ error: 'Missing stylist id' }, 400);
                return handleStylistCalendar(stylistId, env);
            }
...
async function handleStylistCalendar(stylistId: string, env: Env): Promise<Response> {
    const integration = await getStylistIntegration(env.DB, stylistId);
    let calendar: GoogleCalendarClient | null = null;

    const clientId = integration?.google_client_id || env.GOOGLE_CLIENT_ID;
    const clientSecret = integration?.google_client_secret || env.GOOGLE_CLIENT_SECRET;
    const refreshToken = integration?.google_refresh_token || env.GOOGLE_REFRESH_TOKEN;
    const calendarId = integration?.calendar_id || env.GOOGLE_CALENDAR_ID || 'primary';

    if (clientId && clientSecret && refreshToken) {
        calendar = new GoogleCalendarClient({ clientId, clientSecret, refreshToken, calendarId });
    }
    ...
    const dbBookings = (await getBookings(env.DB))
        .filter((b: any) => b.stylist_id === stylistId && b.start_time >= start && b.start_time < end)
        .map((b: any) => ({
            id: b.id,
    ...
    if (calendar) {
        try {
            ...
            for (const event of events) {
                const startTime = event.start?.dateTime || (event.start?.date ? `${event.start.date}T00:00:00+05:30` : null);
                const endTime = event.end?.dateTime || (event.end?.date ? `${event.end.date}T23:59:59+05:30` : null);
            ...
            console.error('Failed to read stylist Google calendar', e);
        }
    }

    const combined = [...googleEvents, ...dbBookings]
